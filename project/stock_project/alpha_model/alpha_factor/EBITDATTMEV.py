...
def EBITDATTMEV(beg_date, end_date):
    ...
    # param
    #################################################################################
    factor_name = 'EBITDATTMEV'
    ipo_num = 90

    # read data
    #################################################################################
    ebitda = Stock().get_factor_h5("EBITDA", None, "primary_mfc")
    ebitda = StockFactorOperate().change_cum_quarter_to_ttm_quarter(ebitda, factor_name="EBITDA")
    ebitda = StockFactorOperate().change_quarter_to_daily_with_report_date(ebitda, beg_date, end_date)
...
if __name__ == '__main__':
    ...
    data = EBITDATTMEV(beg_date, end_date)
